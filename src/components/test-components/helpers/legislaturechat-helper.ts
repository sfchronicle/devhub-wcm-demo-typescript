...
// Scales
const colorBase = scaleOrdinal()
  .domain(['Dem', 'GOP', 'Grn', 'Other'])
  .range(['#003166', '#7F0D0A', '#5F7539', '#B17A00']) // -300
const color = scaleOrdinal()
  .domain(['Dem', 'GOP', 'Grn', 'Other'])
  // .range(['#1874CB', '#D20C0E', '#7CA735', '#B17A00']) // -bright
  .range(['#025BAF', '#BA142D', '#819958', '#E5A72C']) // -100
  .unknown('#fff')
const colorVoting = scaleOrdinal()
  .domain(['Dem', 'GOP', 'Grn', 'Other'])
  .range(['#95C1E8', '#DDABAA', '#C8D3B7', '#F1DEAD']) // -25
  .unknown('#fff')

// Code from Daniel Kao's d3-parliament-chart package
// https://github.com/dkaoster/d3-parliament-chart/blob/main/src/chart-helpers.js
...
export { color, colorBase, colorVoting }
